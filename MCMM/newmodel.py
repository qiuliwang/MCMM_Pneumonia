...
class RCNNMODEL(BaseModel):
    ...
    def build_rnn_txt(self):
        print('**********************\nBuild RNN for Textual Information\n**********************\n')
        self.sex = tf.placeholder(dtype = tf.float32, shape = [1, 1])
        self.age = tf.placeholder(dtype = tf.float32, shape = [1, 1])
        self.complaint = tf.placeholder(dtype = tf.float32, shape = [16, 50])
        print('shape of sex: ', self.sex.get_shape().as_list())
        print('shape of age: ', self.age.get_shape().as_list())
        print('shape of complaint: ', self.complaint.get_shape().as_list())
        ...
        for idx in range(num_steps):
           # Apply the LSTM
            current_input = self.complaint[idx]
            current_input = tf.expand_dims(current_input, 0)
            output, state = lstm(current_input, last_state)
            last_state = state
            outputs.append(output)
        
        outputoftxt = outputs[-1]
        textinfo = tf.concat((outputoftxt, self.age), 1)
        textinfo = tf.concat((textinfo, self.sex), 1)
        self.textinfo = textinfo
        print 'shape of textinfo', self.textinfo.get_shape().as_list()

    def build_rnn(self):
        """ Build the RNN. """
        print('**********************\nBuild RNN for Images\n**********************\n')
        ...
        for idx in range(num_steps):
           # Apply the LSTM
            with tf.variable_scope("lstm"):
                ...
                current_input = tf.expand_dims(current_input, 0)
                current_input = tf.concat((current_input, self.textinfo), 1)
        ...
        # get two lstm outputs concat here
        # twoLstmOutputs = tf.concat((outputs[-1], self.textinfo), 1)
        # print('shape of twoLstmOutputs: ', twoLstmOutputs.get_shape().as_list())
